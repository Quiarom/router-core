...
export function Navbar({ isLive, setIsLive, wanStatus = "connected", activeTab = "assistant", setActiveTab }) {
  return (
    <header className="sticky top-0 z-50 border-b-2 border-neutral-800 bg-black">
      <div className="w-full flex flex-col md:flex-row items-center justify-between px-4 sm:px-6 lg:px-8 py-3 gap-3 relative">
        ...
        <nav className="flex items-center justify-center gap-6 sm:gap-8 font-mono text-xs sm:text-sm md:absolute md:left-1/2 md:-translate-x-1/2">
          <button
            onClick={() => setActiveTab("assistant")}
            className={`py-1.5 border-b-2 font-bold uppercase tracking-wider transition-colors cursor-pointer ${
              activeTab === "assistant"
